...
- Автоматически создает пользователя `sshtunneluser` с полностью заблокированной консолью и терминалом (TTY)
- Создает структуру директории `.ssh` и настраивает права для авторизации по ключам
- Рекурсивно исправляет владельца и права доступа домашней папки, защищая от ошибок ручного переноса ключей
- Безопасно настраивает `/etc/ssh/sshd_config`, изолируя пользователя от системы
    + Разрешает исключительно проброс портов (TCP Forwarding)
    + Запрещает выполнение любых системных команд и интерактивный вход
